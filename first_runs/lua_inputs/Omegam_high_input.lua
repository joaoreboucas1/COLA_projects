...
-- Npart_1D must always be a power of 2
-- If we are only interested in P(k), use force_nmesh = 2*particle_Npart_1D
-- If interested in Halo stuff, use force_nmesh = 3*particle_Npart_1D
force_nmesh = 2048
-- Always fix a seed for reproducibility, it can be anything
ic_random_seed = 1234
-- output_redshifts is a list (with curly braces) of redshifts you want COLA
-- to take a snapshot of particles. Important if you want to recalculate P(k)
-- with another algorithm such as Pylians or Nbodytoolkit
output_redshifts = {3, 2, 1, 0.5, 0.0}
-- Number of (linearly/log)-spaced steps between initial_redshift and z=0
-- if many output_redshifts, then you can specify number of steps between
-- the redshift points. Check for instance arxiv:2203.11120 Table 6
-- That is equivalent to output_redshifts = {3, 2, 1, 0.5, 0} and
-- timestep_nsteps = {12, 5, 8, 9, 17}
-- You can be sparser at high-z and denser at low-z
-- Check impact of z_ini and time steps in arxiv:1509.04685 important paper for COLA resolutions
ic_initial_redshift = 19.0
timestep_nsteps = {12, 5, 8, 9, 17}
-- Whether you want to output particle positions. We are not interested in particles.
output_particles = false
-- Friends of friends algorithm for halo finding. Not interested in halos.
fof = false
fof_nmin_per_halo = 20
-- Internal COLA algorithm to compute P(k).
-- Force calculations and P(k) calculations can have different resolutions.
pofk = true
pofk_nmesh = 2048
-- Pair fix simulations reduce sample variance
...
-- Read transfer info files generated from CAMB
-- You don't need to match redshifts of CAMB-generated T(k) with redshifts here: COLA will spline
-- But you need to make transfers from z = zini to z = 0
-- Good practice: generate transfers a bit before zini
-- Also check the transfer_infofile
ic_type_of_input = "transferinfofile"
ic_input_filename = "/scratch/eubd/joao.reboucas/COLA_projects/first_runs/transfers_from_class/Omegam_high_transferinfo.txt"

------------------------------------------------------------
-- Simulation options
------------------------------------------------------------
-- Which folder to output
-- Try to use absolute paths
output_folder = "/scratch/eubd/joao.reboucas/COLA_projects/first_runs/outputs/Omegam_high"
-- Label
simulation_name = "Omegam_high"
-- Boxsize of simulation in Mpc/h. Use powers of 2!
...
--- CDM density
cosmology_OmegaCDM = 0.311 - 0.0013877839
--- Baryon density
cosmology_Omegab = 0.049
-- Massive neutrino density
cosmology_OmegaMNu = 0.0013877839
-- Dark energy density (a CC)
cosmology_OmegaLambda = 0.638612
...
-- particle_Npart_1D = 256 SET IN BEGINNING
...
-- output_redshifts = {0.0} SET IN BEGINNING
-- Output particles?
-- output_particles = true SET IN BEGINNING
...
-- timestep_nsteps = {20} SET IN BEGINNING
...
------------------------------------------------------------
-- The random seed
-- ic_random_seed = 1234 SET IN BEGINNING
...
-- force_nmesh = 128 SET IN BEGINNING
...
-- fof = false SET IN BEGINNING
-- Minimum number of particles per halo
-- fof_nmin_per_halo = 20 SET IN BEGINNING
...
-- pofk = true SET IN BEGINNING
-- Gridsize to use for this
-- pofk_nmesh = 128 SET IN BEGINNING
